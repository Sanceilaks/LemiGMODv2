...
class DrawFilledBox : public CBaseCall
{
public:
	int x, y, w, h;
	Color color;
	DrawFilledBox(int x, int y, int w, int h, Color color) : x(x), y(y), w(w), h(h), color(color) {}
	virtual void draw() override;

};

class DrawBorderedBox : public CBaseCall
{
public:
	int x, y, w, h, t;
	Color color;
	DrawBorderedBox(int x, int y, int w, int h, int t, Color color) : x(x), y(y), w(w), h(h), t(t), color(color) {}
	virtual void draw() override;
};


/*
functions with render_ - draw using dx
functions with draw_ - draw using surface
*/
class CRenderTool
{
private:
